...
<?php } ?>
							...
							<th width="100"><?php echo $this->gen_image_setting_model->get_label_value("administration", $current_user_language); ?></th>
						</tr>
					</thead>
					<tbody>
						<?php
...
<ul class="pagination">
	<!-- Bu Yazılım İmleç A.Ş. Tarafından Üretilmiştir: http://www.imleç.com.tr -->
		...
		echo "<li><a href='".site_url("gen_image_settings/index/page/".$previous."/gen_module/".$gen_module."/gen_image_setting_type/".$gen_image_setting_type."/auth_user/".$auth_user."/")."'> &larr; önceki </a></li>";
		while ( $i <= $page_count ) {
			if ( $i == 1 ) {
				if ( $i == $page )
					echo "<li class='active'><a href='".site_url("gen_image_settings/index/page/".$i."/gen_module/".$gen_module."/gen_image_setting_type/".$gen_image_setting_type."/auth_user/".$auth_user."/")."'>ilk</a></li>";
				else
					echo "<li><a href='".site_url("gen_image_settings/index/page/".$i."/gen_module/".$gen_module."/gen_image_setting_type/".$gen_image_setting_type."/auth_user/".$auth_user."/")."'>ilk</a></li>";
			}

			if ( $i >= $page-5 && $i <= $page+5  && $i != 1 && $i != $page_count ) {
				if ( $i == $page )
					echo "<li class='active'><a href='".site_url("gen_image_settings/index/page/".$i."/gen_module/".$gen_module."/gen_image_setting_type/".$gen_image_setting_type."/auth_user/".$auth_user."/")."'>{$i}</a></li>";
				else
					echo "<li><a href='".site_url("gen_image_settings/index/page/".$i."/gen_module/".$gen_module."/gen_image_setting_type/".$gen_image_setting_type."/auth_user/".$auth_user."/")."'>{$i}</a></li>";
			}

			if ( $i == $page_count && $page_count != 1 ) {
				if ( $i == $page )
					echo "<li class='active'><a href='".site_url("gen_image_settings/index/page/".$i."/gen_module/".$gen_module."/gen_image_setting_type/".$gen_image_setting_type."/auth_user/".$auth_user."/")."'>son</a></li>";
				else
					echo "<li><a href='".site_url("gen_image_settings/index/page/".$i."/gen_module/".$gen_module."/gen_image_setting_type/".$gen_image_setting_type."/auth_user/".$auth_user."/")."'>son</a></li>";
			}
			$i++;
		}
		echo "<li><a href='".site_url("gen_image_settings/index/page/".$next."/gen_module/".$gen_module."/gen_image_setting_type/".$gen_image_setting_type."/auth_user/".$auth_user."/")."'> sonraki &rarr; </a></li>";
		?>

</ul>
<?php
} else {
	echo '<div class="alert alert-warning"> <strong>'.$this->gen_image_setting_model->get_label_value("no_record_warning", $current_user_language).'</strong>'.$this->gen_image_setting_model->get_label_value("there_is_no_gen_image_settings", $current_user_language) . '</div>';
}
?>
