		...
		<div class="widget box">
			...
			<div class="widget-content">
				<form class="form-horizontal row-border" id="form_to_be_validated"
					action="<?php echo site_url("blob_images/create"); ?>"
					method="post">
					...
					<div class="form-group">
						<label class="col-md-3 control-label"><?php $this->blob_image_model->get_label_value("path", $current_user_language); ?>
						</label>
	...
	if ($('#file_upload_input_field').length) {
		new AjaxUpload('file_upload_input_field', {
			...
			onComplete: function(file, response) {
				...
				if (tmp[1]) {
					var t = "<?php echo base_url().UPLOAD_DIR .'/'.$current_user_role .'/'.$current_user_id.'/'; ?>"+response;
					var path = "<?php echo UPLOAD_DIR .'/'.$current_user_role .'/'.$current_user_id.'/'; ?>"+response;
					$("#data_block").html("<img src="+ t +" width=350 height=350 name=\"img_path\"/>");
					$("#path").val(path);
					this.enable();
					...
					<div class="form-group">
						<label class="col-md-3 control-label" for="entry_id"><?php echo $this->blob_image_model->get_label_value("entry_id", $current_user_language); ?>
						<span class="required">*</span></label>
						<div class="col-md-5">
							<select id="entry_id"
